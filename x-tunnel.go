...
var (
	...
	flagRegistrationOnce sync.Once
)
...
// CLI entrypoints live in separate files. The tunnel core stays importable.
...
func runWebSocketServer(addr string) error {
	...
	if err != nil {
		return fmt.Errorf("[服务端] WS 地址无效: %w", err)
	}
	path := u.Path
	if path == "" {
		path = "/"
	...
	for _, cidr := range strings.Split(cidrs, ",") {
		...
		if err != nil {
			return fmt.Errorf("[服务端] CIDR 解析失败: %w", err)
		}
	...
	if u.Scheme == "wss" {
		...
		if certFile != "" && keyFile != "" {
			...
			if err := server.ListenAndServeTLS(certFile, keyFile); err != nil {
				return fmt.Errorf("[服务端] WSS 启动失败: %w", err)
			}
		} else {
			cert, err := generateSelfSignedCert()
			if err != nil {
				return fmt.Errorf("[服务端] 生成自签名证书失败: %w", err)
			}
		...
		if err := http.ListenAndServe(u.Host, nil); err != nil {
			return fmt.Errorf("[服务端] WS 启动失败: %w", err)
		}
	}
	return nil
}
...
func (p *ECHPool) dialAndServe(idx int, ip string) {
	...
	for {
		...
		if err != nil {
			...
			if !sleepWithContext(p.ctx, 3*time.Second) {
				return
			}
			continue
		}
		...
		if err != nil {
			...
			if !sleepWithContext(p.ctx, cfg.ReconnectDelay) {
				return
			}
			continue
		}
...
func (p *ECHPool) ActiveSessions() int {
	p.wsConnsMu.RLock()
...
func (p *ECHPool) Stop() {
	...
	p.cancel()
	p.wsConnsMu.RLock()
	sessions := make([]*smux.Session, 0, len(p.smuxConns))
	for _, sess := range p.smuxConns {
		if sess != nil {
